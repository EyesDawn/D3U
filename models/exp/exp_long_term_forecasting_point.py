...
from models.exp.exp_basic_point import Exp_Basic
# from models.diffusion_models import diffuMTS
# from models.diffusion_models.diffusion_utils import *
...
# from utils.ddp import is_main_process, get_world_size,gather_tensors_from_all_gpus,cleanup
...
class Exp_Long_Term_Forecast(Exp_Basic):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs, vqloss, _ = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
                        else:
                            outputs, vqloss, _ = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                    if self.args.output_attention:
                        outputs, vqloss, _ = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
                    else:
    ...
    def test(self, setting, test=0, save_result=False, plot=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs, _, _ = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
                        else:
        ...
        res = trues - preds
        print('test shape:', preds.shape, trues.shape, res.shape)
        ...
        if save_result:
            ...
            np.save(folder_path + 'res.npy', res)
