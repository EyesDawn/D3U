...
### Traffic Dataset - Two-stage Training ###
### Stage 1: Pre-training with cond_model_main.py (train iTransformer) ###
### Stage 2: Main training with main.py (train diffusion model) ###

### Usage:
### ./traffic.sh             # Run both stages (pre-training + main training)
### ./traffic.sh main        # Skip pre-training, run only main training
### ./traffic.sh pretrain    # Run only pre-training

# Parse command line argument
MODE=${1:-"both"}  # Default to "both" if no argument provided

echo "========================================="
echo "Starting Traffic Training"
echo "Mode: $MODE"
if [ "$MODE" = "both" ]; then
...
batch_size_main=16
test_batch_size_main=8

## GPU Config ##
gpu_id=6

## Log Paths ##
pretrain_log="./logs/iTrans_M_Traffic_pretrain.log"
main_log="./logs/D3U/iTransformer/Traffic_main.log"
