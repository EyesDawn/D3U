...
### Solar Dataset - Two-stage Training ###
### Stage 1: Pre-training with cond_model_main.py (train iTransformer) ###
### Stage 2: Main training with main.py (train diffusion model) ###

### Usage:
### ./solar.sh             # Run both stages (pre-training + main training)
### ./solar.sh main        # Skip pre-training, run only main training
### ./solar.sh pretrain    # Run only pre-training

# Parse command line argument
MODE=${1:-"both"}  # Default to "both" if no argument provided

echo "========================================="
echo "Starting Solar Training"
echo "Mode: $MODE"
if [ "$MODE" = "both" ]; then
...
## Log Paths ##
pretrain_log="./logs/iTrans_M_Solar_pretrain.log"
main_log="./logs/D3U/iTransformer/Solar_main.log"
