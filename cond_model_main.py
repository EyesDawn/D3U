...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='D3U-Framework-for-Point-MTS-Forecastingg',
        )
    
    # basic config
    parser.add_argument('--is_training', action='store_true', help='status')
    parser.add_argument('--model_id', type=str, default='ETTm2_96_192', help='model id')
    parser.add_argument('--model', type=str, default='VQVAE',
                        help='model name, options: [VQVAE, diffuMTS]')
    ...
    parser.add_argument('--save', action='store_true', help="save test result")
    parser.add_argument('--pos_embed', action='store_true', help="use position embedding")
    ...
    # parser.add_argument('--encoder', type=str, default='PatchTST', help='Encoder of the VQ model')
    parser.add_argument('--decomposition', action='store_true', help='whether to use decomposition') 
    parser.add_argument('--kernel_size', type=int, default=7, help='kernel_size length')
    ...
    parser.add_argument('--decay', type=float, default=0.8, help='decay')
    parser.add_argument('--commitment_weight', type=float, default=1.0, help='commitment_weight ')
    # PatchTST
    parser.add_argument('--fc_dropout', type=float, default=0.05, help='fully connected dropout')
    parser.add_argument('--head_dropout', type=float, default=0.0, help='head dropout')
    # NSformer; de-stationary projector params
    ...
    if args.is_training:
        ...
        exp = Exp(args)  # set experiments
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.test(setting, test=1, save_result=args.save)
        torch.cuda.empty_cache()
